...
def export_to_txt(result_data: Dict[str, Any]):
    ...
    # Handle different result structures (same logic as CSV/Excel)
    if 'transactions' in result_data:
        # Single job result
        transactions = result_data.get('transactions', [])
    elif 'individual_results' in result_data:
        # Bulk job result - combine all transactions from individual results
        for individual_result in result_data.get('individual_results', []):
            if 'transactions' in individual_result:
                transactions.extend(individual_result['transactions'])
    ...
    if not transactions:
        return Response(
            content=f"No transactions found. Structure: {list(result_data.keys())}",
            media_type='text/plain'
        )
