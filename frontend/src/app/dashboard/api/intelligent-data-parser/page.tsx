...
export default function APIPage() {
  ...
  const loadApiKeys = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setApiKeys(keys);
        // Auto-select the first API key if none is selected
        if (keys.length > 0 && !selectedApiKey) {
          const firstKey = keys[0].real_key || keys[0].api_key;
          setSelectedApiKey(firstKey);
          console.log('Auto-selected first API key:', firstKey);
        }
  ...
  return (
    <DashboardLayout title="Intelligent Data Parser API">
      <div className="p-6">
